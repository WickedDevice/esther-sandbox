...
TinyWebServer::TinyWebServer(PathHandler handlers[],
			     const char** headers,
                             const int port)
  : handlers_(handlers),
    server_(WildFire_CC3000_Server(port)),
    path_(NULL),
...
void TinyWebServer::send_error_code(WildFire_CC3000_ClientRef client, int code) {
#if DEBUG
...
boolean TinyWebServer::read_next_char(WildFire_CC3000_ClientRef client, uint8_t* ch) {
  if (!client.available()) {
    return false;
  } else {
    *ch = client.read();
...
int read_chars(TinyWebServer& web_server, WildFire_CC3000_ClientRef client,
